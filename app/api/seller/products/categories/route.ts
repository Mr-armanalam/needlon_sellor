...
export async function GET(request: NextRequest) {
  return routeHandler(async () => {
    ...
    if (categories.length === 0) {
      ...
      // Seed subcategories under Ethnic Wear
      const [kurtis] = await db
        .insert(categoriesTable)
        .values({
          name: "Kurtis",
          slug: "kurtis",
      ...
      const [dupattas] = await db
        ...
        .values({
          ...
          code: "CAT-DUPATTAS",
          parentId: ethnicWear.id,
          path: "/ethnic-wear/dupattas",
          level: 1,
        })
        .returning();

      // Seed main category: Western Wear
      const [westernWear] = await db
        .insert(categoriesTable)
        .values({
          name: "Western Wear",
          slug: "western-wear",
      ...
      const [casualShirts] = await db
        .insert(categoriesTable)
        .values({
          name: "Casual Shirts",
          slug: "casual-shirts",
          code: "CAT-CASUAL-SHIRTS",
          parentId: westernWear.id,
          path: "/western-wear/casual-shirts",
          level: 1,
        })
        .returning();
