...
export default function AddressManagementSection({
  ...
  // Primary structured array holding separate fulfillment address slots
  ...
  // DRAWER INNER FORM CONTENT COMPONENT LAYER
  const RenderAddressFormDrawer = (existingAddr: AddressType | null = null) => {
    const [selectedType, setSelectedType] = useState(
      existingAddr?.type || "Pickup",
    );
      ...
      <div className="space-y-6 h-full flex flex-col justify-between text-xs font-semibold">
        ...
        <div className="pt-4 border-t border-gray-100 flex gap-2 bg-white">
          <button
            type="submit"
            onClick={() =>
              handleAddNewAddress({ type: selectedType, ...formData })
            }
  ...
  // CHECK ACCORDION LENGTH FOR SMART EMPTY STATES
      ...
      <div className="border-b border-gray-100 pb-3 flex items-center justify-between gap-4 flex-wrap">
        <div>
          ...
          <p className="text-xs text-gray-400 mt-0.5">
            Control fulfillment points across courier dispatch, return handling,
            and tax offices.
          </p>
        </div>
        <button
          type="button"
          onClick={() => triggerDrawer(RenderAddressFormDrawer())}
        ...
        {addresses.map((addr) => (
          ...
          >
              ...
              <div className="flex justify-between items-center gap-2">
                ...
                {addr.isVerified && (
                  <span className="text-[10px] text-green-700 font-bold bg-green-50 border border-green-100 px-2 py-0.5 rounded-full flex items-center gap-0.5">
                    <CheckCircle2 className="w-3 h-3 text-green-600" /> Courier
                    Verified
                  </span>
                )}
              </div>
            ...
            <div className="pt-2 border-t border-gray-50 flex justify-end">
              <button
                type="button"
                onClick={() => triggerDrawer(RenderAddressFormDrawer(addr))}
          ...
          <div className="bg-slate-50/60 border border-dashed border-gray-200 rounded-2xl p-5 flex flex-col items-center justify-center text-center space-y-2.5 min-h-[140px]">
            <div className="space-y-1 text-xs">
              ...
              <p className="text-[11px] text-gray-400 font-medium max-w-xs">
                Add a legal address configuration to print business invoice
                files successfully.
              </p>
            </div>
            <button
