...
export default function BusinessIdentitySection({
  ...
  const handleDrop = (e: React.DragEvent<HTMLDivElement>) => {
    ...
    if (e.dataTransfer.files && e.dataTransfer.files[0]) {
      setProfilePhoto(URL.createObjectURL(e.dataTransfer.files[0]));
    ...
    <div className="space-y-6 max-w-4xl animate-in fade-in duration-200">
      ...
      <div className="border-b border-gray-100 pb-3">
        ...
        <p className="text-xs text-gray-400 mt-0.5">
          Your legal business information used for marketplace identity
          management.
        </p>
      </div>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div className="space-y-1.5">
                  ...
                  <select className="w-full bg-gray-50 border border-gray-200 rounded-xl text-xs px-4 py-2.5 font-medium text-gray-700 focus:outline-none focus:ring-2 focus:ring-blue-500/20">
                    <option>Sole Proprietorship / Home-Based Crafter</option>
                    <option>Partnership Firm LLC</option>
                    <option>Private Limited Corporate Entity</option>
                  </select>
                </div>
                <div className="space-y-1.5">
                  <label className="text-[10px] font-bold text-gray-400 uppercase">
          ...
          <div className="bg-white border border-gray-100 rounded-2xl p-5 shadow-xs space-y-4">
            <div className="space-y-1">
              ...
              <p className="text-[11px] text-gray-400 leading-normal">
                Optional now. Provide these identifiers to scale business
                operations later.
              </p>
            </div>
            ...
            <div className="bg-blue-50/30 border border-blue-100/60 rounded-xl p-3.5 space-y-2.5">
              <span className="text-[11px] font-bold text-blue-900 flex items-center gap-1">
                <ShieldCheck className="w-3.5 h-3.5 text-blue-600" /> Account
                Perks Upon Submission
              </span>
              <ul className="space-y-2 text-[10px] text-blue-800 font-medium">
                <li className="flex items-center gap-1.5">
                  <CheckCircle2 className="w-3 h-3 text-blue-600" /> Verified
                  Seller storefront badge link
                </li>
                <li className="flex items-center gap-1.5">
                  <CheckCircle2 className="w-3 h-3 text-blue-600" /> Automated
                  compliance invoice prints
                </li>
                <li className="flex items-center gap-1.5">
                  <CheckCircle2 className="w-3 h-3 text-blue-600" /> Priority
                  domestic tax reporting systems
                </li>
              </ul>
            </div>
          </div>
